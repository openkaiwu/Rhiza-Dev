...
Each Manifest v1 selected item records the actual ResourceVersion ID and digest, exact selection reason, priority, mode, token count and contributor/Planner/Compiler versions. Compiler verifies bytes through the existing BlobStore before publishing the reference. Immutable Manifest insertion and successful message/Run persistence retain the existing transaction boundary. Database triggers reject UPDATE and DELETE; insertion cannot silently replace an existing Manifest. Legacy manifests remain readable with explicit legacy-unversioned resolution.
