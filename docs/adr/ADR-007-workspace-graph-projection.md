...
`workspace_objects` and `graph_relations` are rebuildable projection tables. A per-workspace checkpoint records the applied Domain Journal sequence and semantic checksum. Normal reads reconcile a changed checkpoint into the active projection namespace in the same transaction as its checkpoint. An explicit rebuild writes a fresh immutable namespace, verifies the complete write by completing the transaction, then switches `projection_aliases`; the previous namespace remains available for rollback.

`graph_layouts` and `graph_layout_nodes` own coordinates independently of the semantic object registry. Current legacy node coordinates are migration input and a compatibility fallback only. Archive remains visible as lifecycle metadata, while bounded graph queries include only active relations in traversal.

The HTTP contract exposes neighborhood, path, tree and changes queries. Neighborhood depth is at most 3, results are capped at 500 objects and 2,000 relations, and list queries use cursors. GraphView receives only the UI-facing adapter model, never projection versions, checkpoints, SQL rows or Journal envelopes.
