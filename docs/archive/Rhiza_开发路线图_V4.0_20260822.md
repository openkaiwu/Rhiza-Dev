...
> 本文档与《Rhiza 技术架构设计书 V4.0》共同构成唯一生效的 **Rhiza Architecture & Roadmap Baseline V4.0**。
...
- **Priority**:P1
- **目标**:以固定输入证明 Kernel 可承载下一阶段对象与执行方式(九项兼容性 Spike),把此前观测级性能阈值集中提升为阻断级,正式宣布 Architecture Convergence Complete。
...
- **当前状态**:M01–M10 完成后进入。
- **前置依赖**:M10。
- **具体实现内容**——九项兼容性 Spike(每项固定 fixture digest、执行命令、预期 checksum 与失败分类;只验证 contract,不扩展为产品):
...
- **测试方案 / 故障注入**:九项 Spike contract tests;此前各 Milestone 的故障注入全量复跑。
...
- **非目标**:不把 Spike 代码无审查提升为公开产品 API;不提前实现九项对应的产品能力。
...
11. 九项兼容性 Spike 9/9,性能阈值阻断达标(M11);
...
**阶段明确不做**:Task/Artifact 产品(M19+)、Adaptive Routing 产品(M23)、外部 Agent(M24)、桌面壳(M25)、Multi-Agent(M26+)。
...
- **Architecture Coverage**:§5.3、§11(多 View)、§19(Task Graph 是 Long-term Multi-Agent 的前置)。
...
- **非目标**:自治排程、完整项目管理套件、Assignment 执行(M26+)。
...
- **完成后解锁能力**:长期 Memory/Context Graph(M29)的演进通道。
...
- **非目标**:自动路由、评分引擎黑箱决策、Personal Capability Model(M28)。
- **Architecture State Change**:`执行数据落库但不可见` → `执行可观测,路由有数据辅助`。
- **完成后解锁能力**:Adaptive Routing v2(M28)的数据与 UI 承接面。
...
```mermaid
flowchart TB
  M26[M26 Complex Task Workspace] --> M27[M27 Multi-Agent Orchestration\n+ External Agent Harness]
  M26 --> M29[M29 Memory / Context Intelligence 长期]
  M27 --> M28[M28 Adaptive Routing v2\n+ Personal Capability Model]
  M27 --> M30[M30 Plugin Ecosystem / Registry]
  M28 --> M31[M31 Control Plane / Observability Plane\nPersonal AI Infrastructure]
  M30 --> M31
```

## M26 — Complex Task Workspace(候选)

- **方向**:围绕 Task Graph 组织复杂工作:多 Workstream、依赖、阻塞、Context Packet per task、人机分工视图。
- **基础依赖**:M20(Task 对象)、M22(Context Packet)、M24(外部执行)。
- **启动条件**:M20 的 Task 对象在 dogfood 中出现自然高频使用;至少一个真实复杂项目以 Task 组织完成全程。
- **关键验收方向**:对象/关系 checksum、TaskPlan revision、跨对象 dangling refs = 0、并发 Workstream、真实 Complex Work 使用证据。

## M27 — Multi-Agent Orchestration 与 External Agent Harness(候选)
...
- **基础依赖**:M24(受控外部执行)、M26(Task Graph);Run 的 `assignment_ref/run_group_ref/parent_run_ref` seam。
- **启动条件**:M24 的单 Agent 路径稳定运行且审批/追溯零事故;pause/resume ADR 裁决完成。
...
## M28 — Adaptive Routing v2 与 Personal Capability Model(候选)
...
- **基础依赖**:M23(telemetry 画像与 RoutingDecision 数据形状);足量真实样本。
...
## M29 — 长期 Context / Memory Intelligence 与 Workspace Knowledge(候选)
...
## M30 — Plugin Ecosystem 与 Extension Registry(候选)
...
Stage 2 / Step 3(M26+):Complex Task Workspace → Multi-Agent 编排 →
...
*本路线图是 Rhiza Architecture & Roadmap Baseline V4.0 的里程碑权威。被替代的历史路线见 `docs/archive/`。*
