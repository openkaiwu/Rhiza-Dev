...
> 本文档与《Rhiza 开发路线图 V4.1》共同构成已被 V4.2 取代的 **Rhiza Architecture & Roadmap Baseline V4.1**(不再生效,见文首 Superseded 说明);它曾作为架构决策、开发、Code Review、AI Coding Agent、测试与里程碑验收的统一依据。
...
| --- | --- |
| 本文 | 正式技术架构设计,唯一架构权威 |
| `Rhiza_开发路线图_V4.1_20260824.md` | 正式施工路线,唯一里程碑权威 |
...
Artifact、Task、Knowledge、Decision、Goal、Workstream、Assignment、Handoff、WorkflowDefinition、WorkflowRun、WorkflowGateDecision 等长期对象通过**新增 object type / relation type / event type** 接入,不修改 Kernel 语义。Event Catalog 预留 `artifact.registered` 与 `workflow.*`(§6.5);Graph、Context、Provenance 的 contract 均以 `ObjectRef` 为单位,天然承载新对象族。判定标准见 §19。

Workflow 的 additive contract 可具体表现为 `workflow.contains`、`workflow.instantiates`、`workflow.step_for_task`、`workflow.next`、`workflow.on_success`、`workflow.on_failure`、`workflow.iterates_over` 等 relation type,以及 `workflow.created`、`workflow.started`、`workflow.node_activated`、`workflow.node_completed`、`workflow.node_failed`、`workflow.gate_passed`、`workflow.gate_rejected`、`workflow.iteration_started`、`workflow.paused`、`workflow.resumed`、`workflow.completed` 等 event type。
...
| Workflow Orchestration(Sequence、Parallel、ForEach、Loop、Condition、Gate、HumanApproval、SubWorkflow) | Application-level Workflow Runtime 作为 Task/Execution 协议消费者 | WorkflowDefinition/WorkflowRun/WorkflowGateDecision、Task/Assignment/RunGroup、Domain Journal |
...
Workflow Orchestration 位于 Task Orchestration 之上、Multi-Agent Execution 之上。Workflow Runtime 负责确定性状态机与节点迁移,不把控制流伪装成新的 `ExecutionRun`:

- Task/Agent/Reviewer execution 创建 `ExecutionRun`; `if`、Loop、Gate、Sequence 与 Parallel 的迁移只产生 Workflow 状态和 `workflow.*` Domain Event;
- Gate 结果固定为 `PASS`、`REJECT` 或 `BLOCKED`; `REJECT` 可带反馈重新进入开发节点,`BLOCKED` 必须等待人工介入;
- side-effect dispatch 以 WorkOrder/Assignment 为边界,继续使用现有 `ExecutionRun`、Lease/Fencing 与 RuntimeAdapter,不得绕过 Kernel 的授权和幂等语义;
- `WorkflowRun` 的当前状态、Domain Journal 与 Command Receipt 在同一事务中提交。崩溃后从当前状态恢复,transition command 必须幂等,不得因重启重复发出 side-effect;
- 如果实现 Workflow Orchestration 必须改变上述八类 Kernel contract 的语义,应先回到分层设计并提交 ADR,而不是扩展 Kernel。
...
本文与《Rhiza 开发路线图 V4.1》双向映射。路线图各 Milestone 的 `Architecture Coverage` 字段引用本文章节与不变量;下表为架构侧总览:
...
| §19 Workflow Orchestration seam | M26–M27 |
| §19 其他 Long-term 能力 | M28+(启动条件见路线图) |
...
*本设计书是 Rhiza Architecture & Roadmap Baseline V4.1 的架构权威。被替代的历史文档见 `docs/archive/`。*
