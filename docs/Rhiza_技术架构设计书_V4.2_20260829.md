...
Status: Active baseline; M01–M08 accepted, M09 next
Supersedes: V4.1
```
...
> M01–M08 已有实现与接受证据；V4.2 不要求重新开发。下一开发里程碑是 M09。M04–M08 的可选真实 PostgreSQL 验证在未配置 `DATABASE_URL` 的证据环境中明确记录为 skipped，不视为通过。
