...
| Domain Event Journal（`workspace_events`） | 尚未实现 |
...
| 持久化 `ExecutionRun` 状态机、失败/取消/超时历史 | 尚未实现（仅有请求级 `requestId` 与 SSE 事件） |
...
| Execution Trace Store、独立 Stream Store | 尚未实现 |
...
| Command Envelope / 幂等 Receipt | Command Envelope 已实现并强制 ActorRef/ScopeRef；持久化 Receipt 属于 M05，尚未实现 |
