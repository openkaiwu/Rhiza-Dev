...
describe.skipIf(!databaseUrl)('PostgreSQL migration E2E', () => {
  it('backfills a legacy project into the default workspace exactly once', async () => {
    ...
    try {
      ...
      await apply('0006');
      await apply('0007');
      expect(await migrate(scopedUrl.href)).toEqual(['0008_execution_runs']);
      ...
      expect((await migrationStatus(scopedUrl.href)).map(({ version, applied }) => ({ version, applied }))).toEqual([
        { version: '0001', applied: true }, { version: '0002', applied: true }, { version: '0003', applied: true }, { version: '0004', applied: true }, { version: '0005', applied: true }, { version: '0006', applied: true }, { version: '0007', applied: true }, { version: '0008', applied: true },
      ]);
    } finally {
      await client.end();
